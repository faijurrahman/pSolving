...
#define _N 51
int G[2*_N][2*_N];
long long A[_N][_N], B[_N][_N], temp[_N][_N];

class NegativeGraphDiv1 
{

	public:
	// Combine two matrices. This represents the A * B operation in the explanation 
	vector<vector<long long>> combineMatrices(vector<vector<long long>> A, vector<vector<long long>> B)
	{
		int X=A.size();
		vector<vector<long long>> temp(X, vector<long long>(X,INF));
		loop(i,X) loop(j,X) loop(k,X) MIN(temp[i][j], A[i][k]+B[k][j]);
		return temp;
	}
	...
	long long findMin(int N, vector<int> s, vector<int> t, vector<int> weight, int charges)
	{
		...
		loop(k,X) loop(i,X) loop(j,X) MIN(G[i][j], G[i][k]+G[k][j]);//FloydWarshell

		X=N;
		if(charges==0) return G[0][X-1];

		vector<vector<long long>> A(X, vector<long long>(X,INF));
		loop(i,X) loop(j,X) A[i][j]=G[i][j+X];
		loop(i,X) A[i][i]=min(A[i][i],0LL);

		vector<vector<long long>> B(X, vector<long long>(X,INF));
		loop(i,X) B[i][i]=0;
     
		while(charges>0) 
		{
			if(charges%2==1) B=combineMatrices(B,A);
