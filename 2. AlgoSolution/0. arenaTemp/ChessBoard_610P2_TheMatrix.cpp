...
class TheMatrix 
{
	...
	int MaxArea(vector <string> board) 
	{
		...
		loop(i,X) loop(j,Y)
		{
			if(i!=0 && board[i-1][j] != board[i][j]) dp[1][i][j] = dp[1][i-1][j] + 1;
			else dp[1][i][j] = 1;

			if(j!=0 && board[i][j-1] != board[i][j]) dp[0][i][j] = dp[0][i][j-1] + 1;
			else dp[0][i][j] = 1;
		}
		...
		//		dp[2][i][j] = min(dp[0][i-1][j-1]+1, dp[1][i-1][j-1]+1); 
		//	else dp[2][i][j] = 1;
		//						
		//	res = max(res, dp[2][i][j]*dp[2][i][j]); 
		//}

		//Check maximum rectangle size
		loop(i,X) loop(j,Y)
		{					
			if(i!=0)
			{
				int minHeight = dp[1][i][j];
				loop(k,dp[0][i][j])
				{
					if(k!=0 && board[i-1][j-k] != board[i][j-k+1]) minHeight = 1;
					minHeight = min(minHeight, dp[1][i][j-k]);
					res = max(res, minHeight*(k+1));	
				}
			}
			else 
			{
				res = max(res, dp[1][i][j]*dp[0][i][j]); 
			}		
		}
