...
class TravellingSalesmanEasy 
{
	...
	int getMaxProfit(int M, vector <int> profit, vector <int> city, vector <int> visit) 
	{
		...
		vector<vector<int>> c; c.resize(M); map<int,int> last;
		loop(i,X) c[city[i]-1].push_back(profit[i]);
		loop(i,M){sort(All(c[i])); last[i]=c[i].size()-1;}


		loop(i,Y)
		{
			int ct=visit[i]-1, idx=last[ct];
			if(idx>=0 && c[ct].size()>0) ans+=c[ct][idx];
